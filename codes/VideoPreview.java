...
private void initializePlayer() {
   // vytvorenie standardeho track selektora
   BandwidthMeter bandwidthMeter = new DefaultBandwidthMeter();
   TrackSelection.Factory videoTrackSelectionFactory =
   new AdaptiveTrackSelection.Factory(bandwidthMeter);
   TrackSelector trackSelector =
   new DefaultTrackSelector(videoTrackSelectionFactory);
   //inicializovanie prehravaca
   player = ExoPlayerFactory.newSimpleInstance(this, trackSelector);
   //nastavenie prehravaca v nahlade
   playerView = findViewById(R.id.exoplayer);
   playerView.setPlayer(player);
   // vytvorenie instancie pre media ako je video a hudba
   audio = buildMediaSource(audioUri);
   video = buildMediaSource(videoUri);
   // istancia pre spajanie medii
   ConcatenatingMediaSource mergedSource = new ConcatenatingMediaSource();
   mergedSource.addMediaSource(setMediaSource(audio,video));
   // nastavenie ovladania prehravaca
   player.addListener(eventListener);
   if(mode.equals("trimmedVideo")){
      // vypiname zvuk, ktore samotne video obsahuje
      player.setVolume(0f);
   }
   else if(mode.equals("earlyStop")){
      // spracujeme video na dlzku hudby na pozadi
      player.setPlaybackParameters(new PlaybackParameters(pts));
   }
   player.prepare(mergedSource);

   // prehravac nastavime pre prehravanie
...
private MergingMediaSource setMediaSource(MediaSource audio, MediaSource video) {
   ...
   if(mode.equals("trimmedVideo")){
     // nastavime prehravac pre hudbu na pozadi
     initMediaPlayer();
     // orezane video je potrebne nastavit na danu dlzku ktoru sme si orezali
     // argumenty startMs a endMs ziskavame cez intent
     ...
     // pri rovnakej dlzke videa a hudby nie je potrebne
     // nastavovat druhy prehravac ale staci spojit obe media
